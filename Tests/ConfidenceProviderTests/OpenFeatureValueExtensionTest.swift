...
class OpenFeatureValueExtensionTest: XCTestCase {
    ...
    func testFromNativeDictionary() throws {
        ...
        let openFeatureValue = OpenFeature.Value.fromNativeDictionary(nativeDict)
    ...
    func testFromNativeType() throws {
        ...
        XCTAssertEqual(OpenFeature.Value.fromNativeType("test"), .string("test"))
        XCTAssertEqual(OpenFeature.Value.fromNativeType(true), .boolean(true))
        XCTAssertEqual(OpenFeature.Value.fromNativeType(42), .integer(42))
        XCTAssertEqual(OpenFeature.Value.fromNativeType(Int64(123)), .integer(123))
        XCTAssertEqual(OpenFeature.Value.fromNativeType(3.14), .double(3.14))
        XCTAssertEqual(OpenFeature.Value.fromNativeType(date), .date(date))
        XCTAssertEqual(OpenFeature.Value.fromNativeType(NSNull()), .null)

        // Test array conversion
        let arrayValue = OpenFeature.Value.fromNativeType(["a", 1, true])
        ...
        let dictValue = OpenFeature.Value.fromNativeType(["key": "value", "number": 42])
    ...
    func testRoundTripConversion() throws {
        ...
        let convertedBack = OpenFeature.Value.fromNativeDictionary(dict)
